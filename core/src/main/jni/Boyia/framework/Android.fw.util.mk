LOCAL_SRC_FILES += \
    $(LOCAL_PATH)/framework/util/InputStream.cpp \
    $(LOCAL_PATH)/framework/util/PlatformLib.cpp \
    $(LOCAL_PATH)/framework/util/StringUtils.cpp \
    $(LOCAL_PATH)/framework/util/Utf8StreamReader.cpp \
    $(LOCAL_PATH)/framework/util/TagMap.cpp \
    $(LOCAL_PATH)/framework/util/BoyiaRef.cpp \
    $(LOCAL_PATH)/framework/util/IDCreator.cpp \
    $(LOCAL_PATH)/framework/util/FileUtil.cpp