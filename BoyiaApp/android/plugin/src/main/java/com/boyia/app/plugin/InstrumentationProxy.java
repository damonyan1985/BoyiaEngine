...
public class InstrumentationProxy extends Instrumentation {
    ...
    public ActivityResult execStartActivity(Context who, IBinder contextThread, IBinder token, Activity target,
        ...
        if (infos == null || infos.size() == 0) {
            intent.putExtra(PluginHook.TARGET_INTENT, intent.getComponent().getClassName());
        ...
        try {
            ...
            return (Instrumentation.ActivityResult) execMethod.invoke(mInstrumentation, who, contextThread, token, target, intent, requestCode, options);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public Activity newActivity(ClassLoader cl, String className, Intent intent) throws IllegalAccessException, InstantiationException, ClassNotFoundException {
        String intentName = intent.getStringExtra(PluginHook.TARGET_INTENT);
